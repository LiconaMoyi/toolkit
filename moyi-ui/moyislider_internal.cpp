...
#include "lib/moyistatetransition.h"
...
MoyiSliderStateMachine::MoyiSliderStateMachine(
        MoyiSlider      *slider,
      ...
      m_normalState(new QState(m_sndState))
{
    ...
    MoyiStateTransition *customTransition;
    QPropertyAnimation *animation;

    // Show halo on mouse enter

    customTransition = new MoyiStateTransition(SliderNoFocusMouseEnter);
    ...
    customTransition = new MoyiStateTransition(SliderNoFocusMouseLeave);
    ...
    customTransition = new MoyiStateTransition(SliderChangedFromMinimum);
    ...
    customTransition = new MoyiStateTransition(SliderChangedToMinimum);
...
void MoyiSliderStateMachine::setupProperties()
{
...
MoyiSliderThumb::MoyiSliderThumb(MoyiSlider *slider)
    : MoyiOverlayWidget(slider->parentWidget()),
      m_slider(slider),
      m_diameter(11),
      m_borderWidth(2),
      m_haloSize(0),
...
MoyiSliderThumb::~MoyiSliderThumb()
{
}

bool MoyiSliderThumb::eventFilter(QObject *obj, QEvent *event)
{
    if (QEvent::ParentChange == event->type()) {
        setParent(m_slider->parentWidget());
    }

    return MoyiOverlayWidget::eventFilter(obj, event);
}

void MoyiSliderThumb::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
...
MoyiSliderTrack::MoyiSliderTrack(MoyiSliderThumb *thumb, MoyiSlider *slider)
    : MoyiOverlayWidget(slider->parentWidget()),
      m_slider(slider),
      m_thumb(thumb),
      m_trackWidth(2)
...
MoyiSliderTrack::~MoyiSliderTrack()
{
}

bool MoyiSliderTrack::eventFilter(QObject *obj, QEvent *event)
{
    if (QEvent::ParentChange == event->type()) {
        setParent(m_slider->parentWidget());
    }
...
void MoyiSliderTrack::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event)
